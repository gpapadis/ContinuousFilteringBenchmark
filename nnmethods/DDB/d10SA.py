...
left_df = pd.read_csv(main_dir + 'D10Aemb.csv', sep=deli)
left_df = left_df[['Id', 'Clean Ag.Value']]
left_df.columns = ['id', 'Clean Ag.Value']
left_df["Clean Ag.Value"] = left_df["Clean Ag.Value"].astype(str)
right_df = pd.read_csv(main_dir + 'D10Bemb.csv', sep=deli)
right_df = right_df[['Id', 'Clean Ag.Value']]
right_df.columns = ['id', 'Clean Ag.Value']
right_df["Clean Ag.Value"] = right_df["Clean Ag.Value"].astype(str)
