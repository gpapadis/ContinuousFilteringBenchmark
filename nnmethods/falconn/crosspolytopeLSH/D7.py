...
if __name__ == '__main__':
    ...
    for column in ['Embedded Clean Title','Embedded Clean Ag.Value']:
     for iteration in range(0, 10):
          ...
          outstring = main_dir + "Output/D{}_CP".format(datasetN)+column+datapart+querypart+".txt"
          out = open(outstring, 'a')
          out.write("LSH \t dist Funct \t num tables \t num hash funct \t last cp dim \t num probes \t index time \t query time \t true matches \t all candidates \t all matches \t Index \t Query \t Indexmemory \t Searchmemory \t storage\n")
          out.close()
          for L in range(25,26,1):
              for k in range(1,2):
                  for cpdim in range(1,2):
                      if column == "Embedded Clean Title":
                          number_of_tables = 1
                          cpd = 64
                      ...
                      if column == "Embedded Clean Ag.Value":
                          number_of_tables = 8
                          cpd = 128
                          number_of_functions = 2
                          number_of_probes = 339
                      ...
                      params_cp.distance_function = falconn.DistanceFunction.EuclideanSquared #NegativeInnerProduct
                      ...
                      out = open(outstring, 'a')
                      out.write("{} \t {} \t {} \t {} \t {} \t {} \t {} \t {} \t{} \t {} \t {} \t {} \t {} \t {} \t {} \t {} \n".format(params_cp.lsh_family, params_cp.distance_function, params_cp.l, params_cp.k, params_cp.last_cp_dimension, number_of_probes, constructtime, querytime, finscore, fullCandidates, len(gt), len(dataset), len(queries), indexm, querym, params_cp.storage_hash_table))
                      out.close()
