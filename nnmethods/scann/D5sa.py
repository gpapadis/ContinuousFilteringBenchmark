...
def createAHIndex(data, k, similarity):
    ...
    num_leaves=80, num_leaves_to_search=80, training_sample_size=100).score_ah(dimensions_per_block=2).build()
...
def createBFIndex(data, k, similarity):
    ...
    searcher = scann.scann_ops_pybind.builder(normalized_dataset, k, similarity).tree(
    num_leaves=80, num_leaves_to_search=80, training_sample_size=100).score_brute_force(quantize=False).build()
