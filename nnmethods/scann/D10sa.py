...
def createAHIndex(data, k, similarity):
    ...
    num_leaves=100, num_leaves_to_search=100, training_sample_size=100).score_ah(dimensions_per_block=2).build()
...
def createBFIndex(data, k, similarity):
    ...
    searcher = scann.scann_ops_pybind.builder(normalized_dataset, k, similarity).tree(
    num_leaves=100, num_leaves_to_search=100, training_sample_size=100).score_brute_force(quantize=False).build()
...
if __name__ == '__main__':
    ...
    filenumber = 10                       #which dataset should be used
    ...
    for i in range(len(truthIndex)):
         if not (list(IDIndex).index(truthIndex[i]) in dubIDIndex or list(IDQuery).index(truthQuery[i]) in dubIDQuery):
    ...
    for iteration in range(0, 10):   
        index, indextime, indexmemory = createBFIndex(EmbIndex, k, "dot_product")
