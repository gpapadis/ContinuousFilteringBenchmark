...
def createAHIndex(data, k, similarity):
    ...
    searcher = scann.scann_ops_pybind.builder(normalized_dataset, k, similarity).tree(
       num_leaves=40, num_leaves_to_search=40, training_sample_size=100).score_ah(dimensions_per_block=2).build()
...
def createBFIndex(data, k, similarity):
    ...
    searcher = scann.scann_ops_pybind.builder(normalized_dataset, k, similarity).tree(
       num_leaves=40, num_leaves_to_search=40, training_sample_size=100).score_brute_force(quantize=False).build()
...
if __name__ == '__main__':
    ...
    column = 'Embedded Clean Title'            #column with the embeddings to try
    ...
    for iteration in range(0, 10):         
        index, indextime, indexmemory = createAHIndex(EmbIndex, k, "squared_l2")
