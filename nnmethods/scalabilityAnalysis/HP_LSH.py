...
if __name__ == '__main__':
    ...
    for datasetfile in ['10K', '50K', '100K', '200K', '300K', '1M', '2M']:  #which dataset should be used
        ...
        outstring = main_dir + "Output/" + datasetfile+ "_HP_"+column+".txt"
        out = open(outstring, 'a')
        out.write("LSH \t dist Funct \t num tables \t num hash funct \t num probes \t index time \t query time\t true matches \t all candidates \t all matches \t Index \t mem index \t mem search\n")
        out.close()
        ...
        for iterations in (0, 10):
            ...
            #print('Done')
            #print('{} probes, accuracy {}, score {}'.format(number_of_probes, accuracy, score))
            ...
            out = open(outstring, 'a')
            out.write("{} \t {} \t {} \t {} \t {} \t {} \t {} \t {} \t {} \t {} \t {} \t {} \t {} \n".format(params_cp.lsh_family, params_cp.distance_function, params_cp.l, params_cp.k, number_of_probes, constructtime, querytime, finscore, fullCandidates, len(gt), len(dataset), indexm, querym))
            out.close()
